...
// transaksi yang diminta Owner termasuk baris PEL/nama pelanggan).
'use strict';
global.window = {};
...
// 4. Struktur header wajib: urutan persis INV / TGL / KSR / PEL, satu field per baris.
check('urutan header INV/TGL/KSR/PEL', () => {
  ...
  assert.ok(lines[invIdx + 3].startsWith('PEL : Reno'));
});
...
// 16. TEST CASE WAJIB: pelanggan "Reno" -> baris "PEL : Reno" muncul persis
//     setelah baris "KSR : ...".
check('baris PEL muncul setelah KSR (pelanggan Reno)', () => {
  ...
  assert.strictEqual(lines[ksrIdx + 1], 'PEL : Reno', 'baris PEL tidak persis setelah KSR');
});
...
check('pelanggan lain (bukan hardcode)', () => {
  ...
  assert.ok(lines.includes('PEL : Haryo Susilo'));
});

// 18. TEST CASE WAJIB: transaksi tanpa pelanggan -> "PEL : Umum".
check('tanpa pelanggan -> PEL : Umum', () => {
  const linesEmpty = renderLines(tx({ custName: '' }));
  const linesUndef = renderLines(tx({ custName: undefined }));
  assert.ok(linesEmpty.includes('PEL : Umum'), 'custName kosong string harusnya jadi Umum');
  assert.ok(linesUndef.includes('PEL : Umum'), 'custName undefined harusnya jadi Umum');
});
...
check('nama pelanggan sangat panjang -> truncate, tetap 1 baris, <=48 kolom', () => {
  ...
  const pelLine = lines.find((l) => l.startsWith('PEL : '));
  assert.ok(pelLine, 'baris PEL tidak ditemukan');
  assert.ok(pelLine.endsWith('...'), 'nama panjang seharusnya dipotong dengan "..."');
  const pelLinesCount = lines.filter((l) => l.startsWith('PEL')).length;
  assert.strictEqual(pelLinesCount, 1, 'PEL tidak boleh wrap jadi lebih dari 1 baris');
});
...
check('reprint: PEL tetap nama pelanggan SAAT transaksi dibuat', () => {
  ...
  assert.ok(lines.includes('PEL : Reno'));
...
//     footer) dan "----" (3x: bawah PEL, bawah item, bawah TOTAL) -- sesuai
